...
The application page currently uses a privacy-preserving static-site fallback: after validation, it opens the applicant's email program with a prefilled message. To submit applications directly, set `applicationEndpoint` in `course-data.js` to an HTTPS endpoint that accepts JSON. The endpoint must provide its own server-side validation, spam protection, secure storage, notification workflow, retention policy, and CORS configuration before launch.

The JavaScript emits privacy-safe `jap:analytics` browser events for program views, CTA clicks, application starts, submissions, errors, and FAQ interactions. It forwards them to Plausible only when Plausible is already configured; free-text application answers are never included.
